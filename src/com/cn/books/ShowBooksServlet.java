...
public class ShowBooksServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		list = showBooks.showByPage(page);//调用查询方法
		int maxpage = showBooks.maxpage();//调用最大页数方法
		request.setAttribute("list", list);
		request.setAttribute("page", page);
		request.setAttribute("maxpage", maxpage);
		System.out.println(maxpage);
		request.getRequestDispatcher("webs/books/showallbooks.jsp").forward(request, response);//转发到页面
		out.flush();
		out.close();
	}
