...
<TR>
    ...
    <div id="fenye"> <br />
       <c:if test="${page ==1}">首页</c:if>
    	<c:if test="${page > 1}">
    	<form style="display:inline;" name="form1" method="post" action="/library/SearchServlet"><input type="hidden" name="page" value=1>
    	...
    	<c:if test="${page > 1}">
    	<form style="display:inline;" name="form2" method="post" action="/library/SearchServlet"><input type="hidden" name="page" value="${page-1 }">
    	<input type="hidden" name="bookinfo" value="${bookinfo }">
    	<input type="hidden" name="keyinfo" value="${keyinfo }">
    	<input type="submit" name="button" value="上一页" /></form></c:if>
    	<c:if test="${page == maxpage}">下一页</c:if>
    	<c:if test="${page < maxpage}">
    	<form style="display:inline;" name="form3" method="post" action="/library/SearchServlet"><input type="hidden" name="page" value="${page+1 }">
    	<input type="hidden" name="bookinfo" value="${bookinfo }">
    	<input type="hidden" name="keyinfo" value="${keyinfo }">
    	<input type="submit" name="button" value="下一页" /></form></c:if>
    	<c:if test="${page == maxpage}">末页</c:if>
    	<c:if test="${page < maxpage}">
    	<form style="display:inline;" name="form4" method="post" action="/library/SearchServlet"><input type="hidden" name="page" value="${maxpage }">
    	<input type="hidden" name="bookinfo" value="${bookinfo }">
    	<input type="hidden" name="keyinfo" value="${keyinfo }">
